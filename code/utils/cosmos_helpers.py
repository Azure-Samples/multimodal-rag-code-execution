...
from azure.identity import DefaultAzureCredential  # Import AAD credential
import copy
...
try:
    # Use AAD authentication instead of the primary key
...
class SCCosmosClient:
    def __init__(
        ...
        self.container = init_container(
            container_name, partitionKeyPath, indexing_policy
        )
    ...
    def read_document(self, doc_id, partition_key=None):
        if partition_key is None:
            partition_key = self.partitionKeyPath
        try:
    ...
    def get_document_by_id(self, doc_id, category_id=COSMOS_CATEGORYID):
        query = (
            "SELECT * FROM documents p WHERE p.categoryId = @categoryId AND p.id = @id"
        )
        params = [
            dict(name="@categoryId", value=category_id),
            dict(name="@id", value=doc_id),
        ]
